...
void defos_toggle_fullscreen()
{
    ...
    [window makeKeyAndOrderFront:nil]; // <- Focus is not working without this.
}
...
void defos_create_fullscreen_borderless()
{
    ...
    // This is the only trick works so far.
...
void defos_set_view_size(float x, float y, float w, float h)
{
    ...
    x - viewFrame.origin.x, win_y - viewFrame.origin.x, w + viewFrame.origin.x + windowFrame.size.width - viewFrame.size.width, h + viewFrame.origin.y + windowFrame.size.height - viewFrame.size.height);
...
void* defos_load_cursor_mac(dmBuffer::HBuffer buffer, float hotSpotX, float hotSpotY)
{
    @autoreleasepool
    {
        ...
        NSData* data =
        [[NSData alloc] initWithBytesNoCopy:copy
                                     length:size
                               freeWhenDone:YES];
