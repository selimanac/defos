...
static int getBPPFromModeString(CFStringRef mode) {
  if ((CFStringCompare(mode, CFSTR(kIO30BitDirectPixels),
    ...
    // This is a strange mode, where we using 10 bits per RGB component and pack
    // it into 32 bits Java is not ready to work with this mode but we have to
    // specify it as supported
    return 30;
...
void defos_create_fullscreen_borderless() {
  ...
  // Set the window to borderless and fullscreen
  [window setStyleMask:NSWindowStyleMaskBorderless];
  [window setFrame:screenRect display:YES];
  [window setLevel:NSMainMenuWindowLevel + 1]; // Keep it above the menu bar

  // Ensure it can go full-screen properly
  [window setCollectionBehavior:NSWindowCollectionBehaviorCanJoinAllSpaces |
                                NSWindowCollectionBehaviorFullScreenPrimary];

  // Apply focus workaround
  [window orderOut:nil];
  dispatch_after(
...
void defos_toggle_always_on_top() {
  window.level =
      defos_is_always_on_top() ? NSNormalWindowLevel : NSFloatingWindowLevel;
}

bool defos_is_fullscreen() {
  BOOL fullscreen =
      (([window styleMask] & NSFullScreenWindowMask) == NSFullScreenWindowMask);
...
bool defos_is_maximized() { return is_maximized; }

bool defos_is_always_on_top() { return window.level == NSFloatingWindowLevel; }

void defos_minimize() { [window performMiniaturize:nil]; }
...
void defos_set_view_size(float x, float y, float w, float h) {
  ...
  NSRect rect = NSMakeRect(
      x - viewFrame.origin.x, win_y - viewFrame.origin.x,
...
bool defos_is_cursor_visible() { return is_cursor_visible; }

bool defos_is_mouse_in_view() { return is_mouse_in_view; }
...
WinPoint defos_get_cursor_pos_view() {
  ...
  NSPoint pointInWindow = NSMakePoint(pointInScreen.x - windowOrigin.x,
...
void defos_set_cursor_pos(float x, float y) {
  ...
  if (!is_cursor_locked) {
    CGAssociateMouseAndMouseCursorPosition(
        true); // Prevents a delay after the Wrap call
  }
}
...
void defos_set_cursor_pos_view(float x, float y) {
  ...
  NSPoint pointInWindow =
      [view convertPoint:NSMakePoint(x, view.bounds.size.height - y)
                  toView:nil];
  NSPoint windowOrigin = window.frame.origin;
  NSPoint point = NSMakePoint(pointInWindow.x + windowOrigin.x,
                              pointInWindow.y + windowOrigin.y);
...
bool defos_is_cursor_clipped() { return is_cursor_clipped; }
...
bool defos_is_cursor_locked() { return is_cursor_locked; }

void *defos_load_cursor_mac(dmBuffer::HBuffer buffer, float hotSpotX,
                            float hotSpotY) {
  @autoreleasepool {
    uint8_t *bytes = NULL;
    uint32_t size = 0;
    if (dmBuffer::GetBytes(buffer, (void **)&bytes, &size) !=
        dmBuffer::RESULT_OK) {
    ...
    NSData *data =
        [[NSData alloc] initWithBytesNoCopy:copy length:size freeWhenDone:YES];
...
static DisplayModeInfo parse_mode(CGDisplayModeRef mode,
  ...
  mode_info.scaling_factor =
      (double)mode_info.width / (double)CGDisplayModeGetWidth(mode);
...
static io_service_t IOServicePortFromCGDisplayID(CGDirectDisplayID displayID) {
  ...
  while ((serv = IOIteratorNext(iter)) != 0) {
    ...
    // Otherwise serial number will be nil (= 0) which will match with the
    // output of 'CGDisplaySerialNumber'
...
static char *get_display_name(CGDirectDisplayID displayID) {
  ...
  NSDictionary *deviceInfo = (NSDictionary *)IODisplayCreateInfoDictionary(
      IOServicePortFromCGDisplayID(displayID), kIODisplayOnlyPreferredName);
  NSDictionary *localizedNames = [deviceInfo
      objectForKey:[NSString stringWithUTF8String:kDisplayProductName]];

  if ([localizedNames count] > 0) {
    NSString *screenName = [localizedNames
        objectForKey:[[localizedNames allKeys] objectAtIndex:0]];
    size_t nameLength =
        [screenName lengthOfBytesUsingEncoding:NSUTF8StringEncoding] + 1;
    char *name = (char *)malloc(nameLength);
    [screenName getCString:name
...
void defos_get_displays(dmArray<DisplayInfo> &displayList) {
  ...
  for (int i = 0; i < numDisplays; i++) {
    ...
    if (CGDisplayIsInMirrorSet(displayID) &&
        CGDisplayPrimaryDisplay(displayID) != displayID) {
      continue;
    }
...
void defos_get_display_modes(DisplayID displayID_,
  ...
  for (int i = 0; i < modeCount; i++) {
    ...
    for (int j = 0; j < i; j++) {
      ...
      CFStringRef otherPixelEncoding =
          CGDisplayModeCopyPixelEncoding(otherMode);

      bool samePixelEncoding =
          (CFStringCompare(pixelEncoding, otherPixelEncoding, 0) ==
           kCFCompareEqualTo);
      CFRelease(pixelEncoding);
      CFRelease(otherPixelEncoding);

      if (samePixelEncoding && pixelWidth == otherPixelWidth &&
          pixelHeight == otherPixelHeight && width == otherWidth &&
          height == otherHeight && refreshRate == otherRefreshRate) {
        shouldAdd = false;
        break;
      }
    }
...
DisplayID defos_get_current_display() {
  return (void *)(size_t)[[[[window screen] deviceDescription]
